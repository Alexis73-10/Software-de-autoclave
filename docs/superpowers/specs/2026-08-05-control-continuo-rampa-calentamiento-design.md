...
**Alcance:** `src/autoclave/state_machine/cycle_phases/calentamiento.py` — reemplaza los pasos 4 y 5 de `update()` (gate de entrada a PWM_ACTIVO y control de `vapor_camara`). No modifica `ESTABLE_PREESTERILIZACION` (paso 7), los escapes (paso 6), el cálculo de pendiente (paso 3) ni ninguna otra fase — el paso 4 sí **lee** (no escribe) la bandera `self._en_sostenimiento` que el paso 7 ya mantenía, agregado en la corrección de revisión final (sección 3.3).

**Reemplaza a:** `docs/superpowers/specs/2026-08-05-fix-overshoot-calentamiento-design.md`. De sus tres mecanismos, el **mecanismo 1** (gate anclado a `p_obj`/`t_obj`) ya se implementó por separado y está en `dev` (commits `211f2fd`, `5201370`) — este diseño lo reemplaza otra vez, de raíz: en vez de corregir el gate discreto, lo elimina y lo reemplaza por una ley de control continua (sección 2). El **mecanismo 2** (tope de temperatura al 97% con espera de presión) no está implementado todavía y se conserva aquí como tercer término del duty (`duty_calidad_vapor`, sección 3.3). El **mecanismo 3** (techo independiente de presión) tampoco está implementado y se conserva como resguardo de seguridad (sección 5).
...
**Corrección post-implementación (2026-08-05, revisión final de branch):** la revisión final del plan ya implementado encontró tres problemas de composición, no visibles en las revisiones por tarea porque cada una vio solo su propio diff:

1. **`duty_calidad_vapor` seguía corriendo dentro de `ESTABLE_PREESTERILIZACION`** (paso 7). La sección 3.3 original afirmaba que `temp < t_obj` por construcción mientras la pausa está activa — falso: nada acota `temp` por arriba una vez que la fase entró en la banda de sostenimiento (`temp` puede estar en `[t_obj, t_obj + rango_temp_estabilizacion]`). Confirmado con datos reales del ciclo 72 (`data/autoclave.db`): la máquina pasó buena parte de los últimos ~4°C de ese ciclo, y el punto de entrega mismo, exactamente en la región donde esto fuerza `duty=0` — con el escape lento enclavado abierto, eso es fuga sin reposición dentro del propio tramo de sostenimiento. Corrección: sección 3.3 ahora excluye explícitamente este término mientras `self._en_sostenimiento` es `True`.
2. **`intervalo_segmentos_calor = 0` (valor permitido por la UI) anulaba los cuatro mecanismos de seguridad** — con `t_on=0` y `t_off=0`, `_tick_dos_estados` cae en la rama "enclavada abierta" sin importar el `duty` calculado. Corrección: sección 3.4 agrega un camino directo que aplica `duty` sin pasar por el temporizador cuando `intervalo <= 0`.
3. **Cero observabilidad de la nueva ley de control** — sin el log del gate discreto que reemplaza, un estancamiento en `duty=0` (por cualquiera de los cuatro mecanismos) no deja rastro hasta el `FALLO` final, replicando el problema de diagnóstico original del ciclo 72. Corrección: nueva sección 6 (Observabilidad).
...
Sin restricción (`1.0`) mientras `temp` no llegó al 97% de `t_obj`. Al cruzar ese umbral, exige que la presión ya "corresponda" a la temperatura actual (`P_sat(temp) + p_add`, el mismo criterio que define `p_obj` pero evaluado con la temperatura real en vez de la fija) — si no, corta el duty a `0.0` por completo (pausa binaria, igual que el mecanismo original; no es una interpolación continua porque lo que se verifica es una condición física de sí/no, no una distancia). `p_min_para_temp` se recalcula cada tick con la temperatura real, nunca se congela al cruzar el 97%. No genera `FALLO` ni tiene timeout propio — si la presión nunca "alcanza" a la temperatura, el ciclo queda pausado hasta el timeout general de `timeout_calentamiento`, sin cambios.

**Corrección (revisión final):** este término **no se aplica una vez que la fase ya entró a `ESTABLE_PREESTERILIZACION`** (`self._en_sostenimiento is True`):
...
La afirmación original de esta sección — que `temp < t_obj` por construcción mientras la pausa está activa, y por lo tanto nunca se solapa con el paso 7 — es falsa: `_en_sostenimiento` se activa con `temp >= t_obj and pres >= p_obj` (paso 7), y desde ahí `temp` puede seguir subiendo hasta `t_obj + rango_temp_estabilizacion` por inercia térmica, dentro de la propia banda tolerada de sostenimiento. En esa banda, `duty_calidad_vapor` sí puede evaluar `temp >= temp_cap` con `pres` momentáneamente por debajo de `P_sat(temp)+p_add` (margen térmico real de la máquina, no un caso de laboratorio — confirmado con datos del ciclo 72) y forzar `duty=0` **dentro del tramo que se supone ya está estable**, con el escape lento enclavado abierto y sin reposición de vapor: exactamente el tipo de fuga sostenida que `ESTABLE_PREESTERILIZACION` ya maneja con su propio timeout de recuperación (paso 7, sin cambios) — no hace falta que `duty_calidad_vapor` la duplique con un corte más agresivo e invisible. Fuera de la banda de sostenimiento, el comportamiento descrito arriba no cambia.
...
if intervalo <= 0:
    # Corrección (revisión final): con intervalo<=0, t_on=t_off=0 y
    # _tick_dos_estados cae en su rama "enclavada abierta" sin importar
    # duty -- los cuatro mecanismos de seguridad (tasa, proximidad, calidad
    # de vapor, techo) quedarian anulados por un solo parametro de ciclo
    # mal configurado (la UI permite intervalo_segmentos_calor=0). Aplicar
    # duty directo, sin pasar por el temporizador de pulsos.
    if duty > 0.0:
...
Se evalúa en cada tick, reemplazando por completo los pasos 4 y 5 actuales (gate de entrada + bang-bang/PWM condicional). Reutiliza el mismo helper `_tick_dos_estados` que ya usan PWM_ACTIVO y `esterilizacion.py`, sin cambios en ese helper, salvo el camino directo agregado para `intervalo <= 0`. Como `t_on`/`t_off` se recalculan en cada tick a partir de datos vivos, un pulso en curso puede acortarse o extenderse frente a lo que se calculó al iniciarlo — es intencional: da respuesta más fina que esperar a que termine el periodo completo del pulso.

Nota: el techo independiente (antes sección 5, evaluado por separado) se movió acá arriba en el orden de ejecución porque el camino de `intervalo<=0` necesita el valor final de `duty` (con el techo ya aplicado) antes de decidir ON/OFF. La sección 5 sigue describiendo la semántica del techo; el bloque de código de esta sección 3.4 es ahora la referencia completa del orden real de evaluación.
...
Ni `duty_proximidad` ni `duty_calidad_vapor` bajan a `0.0` de forma permanente una vez cruzado el objetivo — `duty_proximidad` se estabiliza en `duty_estable` (p. ej. 30% si `factor_calentamiento=70`) y `duty_calidad_vapor` vuelve a `1.0` en cuanto la presión "alcanza" a la temperatura (o directamente no se evalúa dentro de `ESTABLE_PREESTERILIZACION`, ver corrección en 3.3). Si la inercia térmica empuja la presión bien por encima de `p_obj` mientras `duty_tasa` no lo detecta (pendiente ya no creciendo en el momento evaluado), el controlador seguiría inyectando vapor en vez de cortar. Se retiene el mecanismo 3 del spec reemplazado como resguardo (`p_techo = p_obj + p_add`, fuerza `duty = 0.0` sin importar el resto) — código real y orden de evaluación en la sección 3.4. No es parte de la ley de control proporcional — es un corte duro de emergencia, igual que en el diseño original. Reutiliza `p_add`, ya en alcance; no agrega parámetros.

## 6. Observabilidad (agregado en revisión final)

Sin reemplazo alguno del log de gate que existía antes (`"objetivo cercano — entra a PWM_ACTIVO"`), un estancamiento en `duty=0` por cualquiera de los cuatro mecanismos no deja rastro hasta el `FALLO` por timeout — el mismo problema de diagnóstico que motivó investigar el ciclo 72 originalmente, ahora aplicado a una ley de control que ya no tiene tramos discretos que loguear en sus transiciones. Se agrega logging edge-triggered (no por tick, para no saturar el log) en las transiciones de `duty` a/desde `0.0`:

```python
duty_anterior = self._duty_actual
# ... cálculo de duty_tasa, duty_proximidad, duty_calidad_vapor, duty, techo ...
self._duty_actual = duty

if duty <= 0.0 and (duty_anterior is None or duty_anterior > 0.0):
    razones = []
    if duty_tasa <= 0.0: razones.append("tasa")
    if duty_proximidad <= 0.0: razones.append("proximidad")
    if duty_calidad_vapor <= 0.0: razones.append("calidad_vapor")
    if pres >= p_techo: razones.append("techo")
    logger.warning(
...
`duty_anterior` se lee de `self._duty_actual` (ya existe desde la sección 3.4) antes de sobreescribirlo. El log de razones es informativo, no exhaustivo por prioridad: cuando más de un término está en `0.0` simultáneamente, se listan todos. No es una fuente de verdad para tests (el atributo `self._duty_actual` sigue siendo eso); es exclusivamente para diagnóstico en campo.
...
- `self._en_sostenimiento is True` → `duty_calidad_vapor = 1.0` sin evaluar la condición anterior, sin importar `temp`/`pres` (corrección de revisión final, sección 3.3).
- `intervalo_segmentos_calor <= 0` → `duty` se aplica directo a `vapor_camara_on/off` sin pasar por `_tick_dos_estados` (corrección de revisión final, sección 3.4); `duty=0` sigue forzando OFF, a diferencia del comportamiento previo a la corrección donde `intervalo<=0` forzaba ON sin importar `duty`.
- Sensor `None` → ya se corta en el paso 2 existente, antes de llegar a este cálculo; sin cambios.

## 8. Qué NO cambia

- `ESTABLE_PREESTERILIZACION`: su gate de entrada, ventana continua de sostenimiento y timeout de recuperación (`timeout_recuperacion_estabilizacion`) — la lógica del paso 7 en sí no se modifica.
...
**Acoplamiento nuevo, acotado (revisión final):** el paso 4 ahora **lee** `self._en_sostenimiento` (bandera que el paso 7 escribe) para decidir si evalúa `duty_calidad_vapor` — es una lectura de un solo booleano, no una modificación del paso 7 ni de su máquina de estados. Es el único punto de acoplamiento entre ambos pasos; todo lo demás en esta lista sigue siendo válido tal cual.
...
- El estado `_en_pwm` y el log "objetivo cercano — entra a PWM_ACTIVO" desaparecen; cualquier código o test que dependa de ese estado interno debe actualizarse (ver sección 10).
- El log de observabilidad (sección 6) es diagnóstico, no una fuente de verdad para tests ni para lógica de control — no dispara `FALLO` ni cambia comportamiento.
- `rango_calentamiento` está declarado en la UI/JSON como parámetro en kPa (heredado de su rol anterior como umbral de presión del gate) y ahora también se usa como margen de distancia en `duty_proximidad` para la variable de temperatura (°C). Con el valor por defecto (2.0) la dirección del error es conservadora (banda de interpolación angosta en presión, casi un escalón), pero la unidad mostrada ya no describe fielmente el parámetro. Aceptado como está — retunear o partir en dos parámetros (uno por variable) violaría la restricción de "ningún parámetro nuevo" de este plan; queda para una revisión de UI/parámetros aparte si se decide encarar.
- El duty "continuo" queda cuantizado por el periodo del tick del loop de control (~0.5s) y por `intervalo_segmentos_calor`: valores de duty muy bajos (por debajo de ~1 tick / intervalo) se redondean al pulso mínimo representable. No compromete los extremos (`duty=0`/`duty=1` siguen siendo exactos vía las ramas `t_on<=0`/`t_off<=0` de `_tick_dos_estados`), solo reduce la resolución efectiva del control intermedio.
...
- **Nuevo (revisión final):** `duty_calidad_vapor` no restringe (queda en `1.0`) cuando `self._en_sostenimiento is True`, aunque `temp`/`pres` estén en una combinación que forzaría `0.0` fuera de esa banda — reproducir con datos equivalentes a los del ciclo 72 dentro de la banda de sostenimiento.
- **Nuevo (revisión final):** con `intervalo_segmentos_calor=0` y `duty=0.0` (por cualquiera de los cuatro mecanismos), `vapor_camara_off()` se llama y `vapor_camara_on()` no — a diferencia del comportamiento previo a la corrección.
- **Nuevo (revisión final):** con `intervalo_segmentos_calor=0` y `duty=1.0`, `vapor_camara_on()` se llama continuamente (caso sano, sin regresión).
- **Nuevo (revisión final):** el log de observabilidad se emite (`caplog` o mock de `logger`) en la transición de `duty>0` a `duty=0` y viceversa, y no se repite en ticks consecutivos con el mismo signo de `duty`.
...
- Casos borde de la sección 7 (`rango_calentamiento=0`, `factor_calentamiento` en `0`/`100`, tasas deshabilitadas, `duty_calidad_vapor` antes/después del 97% y dentro/fuera de sostenimiento, `intervalo<=0`).
- **Restituir** un equivalente a `test_tasa_excedida_muchos_ticks_consecutivos_nunca_produce_fallo` (eliminado en la Tarea 2 sin reemplazo, señalado en la revisión final): un test de integración que confirme que `tasa_calentamiento`/`tasa_presion` excedidas durante muchos ticks consecutivos nunca producen `FaseResult.FALLO` — los tests puros de `_duty_por_tasa` no verifican `FaseResult`, solo el valor de duty.
...
## 11. Documentación a actualizar (en la implementación)
